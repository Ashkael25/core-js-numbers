...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const square = Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(sum / square);
}
...
function roundToSmallestInteger(number) {
  return Math.floor(number).toFixed();
}
...
function roundToLargestInteger(number) {
  return Math.ceil(number).toFixed();
}
...
function roundToNearestInteger(number) {
  return Math.round(number).toFixed();
}
...
function getIntegerPartNumber(number) {
  return Math.trunc(number).toFixed();
}
